...
static void traffic_gen(struct rte_mempool *mp) {    
    for (int i = 0; i < 1000; i++) {
        struct rte_mbuf *paket = rte_pktmbuf_alloc(mp);
        if (!paket) continue;
        
        rte_pktmbuf_append(paket, 64);
        
        uint8_t *data = rte_pktmbuf_mtod(paket, uint8_t *);
        for (int j = 0; j < 64; j++) {
            data[j] = rand() & 0xFF;
        }
        rte_eth_tx_burst(0, 0, &paket, 1);
    }
}
		...
		struct rte_mbuf **pkts, uint16_t nb_pkts, void *_ __rte_unused)
{
	...
	if (latency_numbers.total_pkts > (1000)) {
...
lcore_main(void)
{
    ...
    const uint64_t MAX_PACKETS = 10000;
	...
	printf("\nCore %u forwarding packets. [output when processing %"PRIu64" packets]\n",
			rte_lcore_id(), MAX_PACKETS);
	for (;packet_counter < MAX_PACKETS;) {
		RTE_ETH_FOREACH_DEV(port) {
	...
	printf("Processed %"PRIu64" packets. Exiting.\n", packet_counter);
	rte_eal_cleanup();
	exit(0);
}
